...
from app.api.userStory.enums import UserStoryStatus
from app.db.db import Base, Defaults
from app.api.room.enums import  Methdology, ProjectStatus, RoomMemberRole, RoomMemberStatus, SprintStatus, Visibility, WorkItemType
...
class Sprint(Base, Defaults):
    ...
    status: Mapped[str] = sa.Column(sa.String, nullable=False, default=SprintStatus.PLANNED)  
...
# TODO should add visibility column to con form if allowed to be public could
...
class UserStory(Base, Defaults):
    ...
    status: Mapped[str] = sa.Column(sa.String, nullable=False, default=UserStoryStatus.NOT_STARTED)  # type: ignore
    ...
    theStory = sa.Column(sa.JSON, nullable=True)
    # TODO should be take index key here in the sprint_id. to be optimaze in the performance.
    ...
    backlog_id: Mapped[str] = sa.Column(sa.ForeignKey("backlogs.id"), nullable=False)
    backlog = relationship("Backlog", back_populates='user_story')
