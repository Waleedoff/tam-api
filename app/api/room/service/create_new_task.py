...
def create_new_task_(room_id: str, body: TodoCreateRequest, session: Session, current_user: UserResponse):
    ...
    task = Todo(**body.model_dump(),user_id=user.id, created_by=current_user.email, room_id=room_id)
    session.add(task)
