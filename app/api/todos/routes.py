...
from app.api.todos.services.create_new_task import create_new_task_
...
from app.api.todos.schema import TaskStatistics, TodoCreateRequest, TodoEmpResponse, TodoResponse
...
from app.api.todos.services.get_tasks_by_user_story_id import  get_tasks_by_user_story_id_
...
def get_tasks_by_user_story_id(user_story_id: str, session: Session = db_session) -> list[TodoEmpResponse]:
    return get_tasks_by_user_story_id_(user_story_id=user_story_id, session=session)


@router.get('/preview/workspace')
def get_all_taks( session: Session = db_session)-> list[TodoResponse]:
    return get_all_tasks_(session=session)
...
@router.get('/')
def get_taks( session: Session = db_session, current_user: UserResponse = Depends(get_current_active_user))-> list[TodoResponse]:
    return get_tasks_(current_user=current_user, session=session)




@router.post('/')
def create_new_task(body: TodoCreateRequest, session: Session = db_session, current_user: UserResponse = Depends(get_current_active_user)):
    return create_new_task_(body=body, session=session, current_user=current_user)
