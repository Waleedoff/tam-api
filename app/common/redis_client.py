...
class RedisClient:
    ...
    def get_data(self, key) -> JSONType | None:
        return None

    def mget_data(self, keys: list[str]) -> list[JSONType | None] | None:
        return None
...
def get_redis_client(config: BaseConfig):
    ...
    local_cash: TTLCache = TTLCache(
        maxsize=config.REDIS_LOCAL_CASH_SIZE_LIMIT, ttl=config.REDIS_LOCAL_CASHING_TTL
    )
    lock = Lock()
    ...
    class RealRedisClient(RedisClient):
        ...
        def mget_data(self, keys: list[str]) -> list[JSONType | None]:
            ...
            none_keys = [
                key for key, data in zip(keys, data_list, strict=False) if data is None
            ]
            ...
            for key, data in zip(none_keys, res, strict=False):
